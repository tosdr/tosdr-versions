...
Kids & Teens
 ...
 ![ Homeware](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7)Homeware
...
A cookie is a text-only string of information that the Site’s server transfers to the cook-ie file of the browser on your computer or other device so that the Site’s server can read and retrieve the contents of that cookie.

2\. HOW WE USE COOKIES
----------------------

### 2.1 We currently use both session and persistent cookies on our sites.

Below are the major cookies used, where the relevant features are enabled on a par-ticular Site.

| Name | Owner (EMP/ 3.rd party) | Description / Purpose | Expiration |
| --- | --- | --- | --- |
| LastTracker | EMP | Stores the name of the last tracker a customer came in contact with. This is used to determine which channel wins the attribution. | 1 Month |
| **uslk\_e** | Userlike | Stores an encoded value and some user tracking information. It's used to recognize users after they terminated the conversation. | 1 Year |
| **uslk\_s** | Userlike | Stores the application data for UserLike. It's used to persist data and store the machine state as nessessary data to enable the application. | Session |
| **\_\_cq\_dnt** | Demandware | Stores a user tracking data. It's used to track and analyze users who are visiting our page anonymous or are browsing in a logged-in state. | Session |
| **\_\_cfduid** | Demandware | Stores a user tracking data. It's used to track and analyze users who are visiting our page anonymous or are browsing in a logged-in state. | 30 Days |
| **\_\_cq\_bc** | Demandware | Stores a user tracking data. It's used to track and analyze users who are visiting our page anonymous or are browsing in a logged-in state. | 30 Days |
| **\_\_cq\_seg** | Demandware | Stores a user tracking data. It's used to track and analyze users who are visiting our page anonymous or are browsing in a logged-in state. | 13 Months |
| **\_\_cq\_uuid** | Demandware | Stores a user tracking data. It's used to track and analyze users who are visiting our page anonymous or are browsing in a logged-in state. | 12 Months |
| **cqcid** | Demandware | Stores a user tracking data. It's used to track and analyze users who are visiting our page anonymous or are browsing in a logged-in state. | Session. |
| **dtCookie** | Dynatrace | Stores a list of parameters in the form of key-value pairs. They are used to track a visit over the span of multiple requests. | Session |
| **dtLatC** | Dynatrace | Stores a numeric value. It's used to measure the server latency for performance monitoring. | Session |
| **dtPC** | Dynatrace | Stores value generated by Dynatrace. It's used to track users around the site. | Session |
| **dtSa** | Dynatrace | Stores an action parameter. It's used to temporarily store page spanning actions. | Session |
| **dw\_dnt** | Demandware | Stores the session ID. Is used to check if first-party cookies can be used and are used to determine if a session shouldn't be tracked anymore. | Session |
| **dwac\_\*** | Demandware | Stores user information, cart content and e-commerce related information. It's used to manage the website and provide basic functionality. | 24 Month |
| **dwanonymous\_\*** | Demandware | Stores user information, cart content and e-commerce related information. It's used to manage the website and provide basic functionality. | 6 Month |
| **dwsecuretoken\_\*** | Demandware | Stores security information. It's used to store the necessary information to secure the connection via HTTPS. | Session |
| **dwsid** |     | Stores security information. It's used to store the necessary information to secure the connection via HTTPS. | Session |
| **emp\_lt\_wt\_mc** | EMP | Stores encoded tracker information. The information is used to track and analyze user movement and behavior across the page. | 1 Month |
| **rxVisitor** | Dynatrace | Stores the from Dynatrace generated Visitor ID. It's used to correlate sessions to a unique visitor. | Permanent |
| **rxvt** | Dynatrace | Stores two timestamps. These are used to determine the session timeout, defined by Dynatrace. | Session |
| **session-set** | EMP / Demandware | Stores a true or false value. It's used to persist the information if a session was set. | 20 Years |
| **sfcc\_languageHint** | EMP | Stores a true or false value. It's used to control if a langue selector should be shown or hidden. | 10 Years |
| **sid** | Demandware | Stores an encoded value. It's used to uniquely identify the user's session. | Session |
| **dtbd98** | EMP | Stores the content of the wishlist. It's used to allow anonymous users to save products on their wishlist. | 60 Days |
...
* Glami AddToCart - This tracker transmits sales information such as order value and item number to our partner. The partner uses this information to analyze and optimize the traffic that comes to us through his site.

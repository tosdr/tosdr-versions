...
为了保障服务的稳定性与安全性，我们将收集您的硬件型号、操作系统版本号、设备名称、设备配置、国际移动设备识别码、唯一设备标识符、网络设备硬件地址、IP 地址、WLAN 接入点、IMSI、WIFI 扫描结果（WIFI 列表）、WIFI 的 SSID 及 BSSID、蓝牙、基站、软件版本号、网络接入方式、类型、状态、网络质量数据、操作、使用、服务日志、SIM 卡序列号（如 ICCID）。我们可能会在后台运行状态下收集您的上述信息，也可能将您的设备信息或电话号码与您的知乎账号相关联，以用于身份验证、安全防范、诈骗监测、预防或禁止非法活动、降低风险、存档和备份用途。

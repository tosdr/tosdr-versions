[![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_1080,q_auto/Brand/Logo%20Section/Mark/Mark_01)](https://status.im/)

Features

Ecosystem

Organization

Help

Collaborate

Developers

SNT

Mobile early accessBeta 2.0 for macOS[Beta 2.0 for Windows](https://status.im/api/download/windows)[Beta 2.0 for Linux](https://status.im/api/download/linux)

[![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_1080,q_auto/Brand/Logo%20Section/Mark/Mark_01)](https://status.im/)

* Features
* Ecosystem
* Organization
* Help
* Collaborate
* Developers
* SNT

Beta 2.0 for macOS[Beta 2.0 for Windows](https://status.im/api/download/windows)[Beta 2.0 for Linux](https://status.im/api/download/linux)Mobile early access

[![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_1080,q_auto/Brand/Logo%20Section/Mark/Mark_01)](https://status.im/)

* Features
* Ecosystem
* Organization
* Help
* Collaborate
* Developers
* SNT

Beta 2.0 for macOS[Beta 2.0 for Windows](https://status.im/api/download/windows)[Beta 2.0 for Linux](https://status.im/api/download/linux)Mobile early access

[![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_1080,q_auto/Brand/Logo%20Section/Mark/Mark_01)](https://status.im/)

Features

Ecosystem

Organization

Help

Collaborate

Developers

SNT

Beta 2.0 for macOS[Beta 2.0 for Windows](https://status.im/api/download/windows)[Beta 2.0 for Linux](https://status.im/api/download/linux)Mobile early access

[New

A New Dawn for Status and SNT: Embracing Decentralisation](https://status.im/blog/new-dawn-for-status-and-snt-embracing-decentralisation)

Make the  
jump to web3
=======================

Use the open source, decentralised crypto communication super app.

Mobile early accessBeta 2.0 for macOS[Beta 2.0 for Windows](https://status.im/api/download/windows)[Beta 2.0 for Linux](https://status.im/api/download/linux)

iOS & Android betas coming soon.  
Desktop beta 2.0 is live!

Status Desktop Beta

### Get Status  
for Desktop

Try Status Desktop’s public beta release today.

Beta 2.0 for macOS[Beta 2.0 for Windows](https://status.im/api/download/windows)[Beta 2.0 for Linux](https://status.im/api/download/linux)

Beta releases may occasionally contain bugs and issues.

Report bugs on [GitHub](https://github.com/status-im/status-desktop/issues/new?assignees=&labels=bug&projects=&template=bug.md)

Post and upvote feature requests [here](https://status.im/feature-upvote)

![Desktop screenshot showing the wallet feature included in the Status app](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_3840,q_auto/Platforms/Screens/Desktop%20Screens/Wallet/Wallet)

Communities

Discover your community
-----------------------

Find your tribe in the metaverse of truly free Status Communities.

[Learn more](https://status.im/features/communities)

### Explore the universe of self-sovereign communities.

![Mobile app screenshot showing the discover community feature through which the user can discover and join new communities](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_1920,q_auto/Homepage/Screens/Communities%20Feature%20Section/Communities_01)

### Decentralised and permissionless.

![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_1920,q_auto/Homepage/Illustrations/Communities%20Feature%20Section/01_Communities_Card)

### Access token-gated channels. Become eligible for airdrops.

![Bottom portion of a mobile app screenshot showing the possibility to join token gated channels by hodling tokens, collectibles and more](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_1920,q_auto/Homepage/Screens/Communities%20Feature%20Section/Communities_02)

![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_3840,q_auto/Textures/screen-texture)

Create community

Take back control
-----------------

Don’t give Discord or Telegram power over your community.

[Set your community free](https://status.im/features/create-community)

Messenger

Chat privately with friends
---------------------------

Protect your right to free speech with decentralised messaging, metadata privacy and e2e encryption.

[Learn more](https://status.im/features/messenger)

![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_256,q_auto/Homepage/Stickers/01)![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_256,q_auto/Homepage/Stickers/02)

![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_3840,q_auto/Textures/screen-texture)

### Create and join unstoppable group chats.

![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_3840,q_auto/Homepage/Illustrations/Messenger%20Feature%20Section/01_Messenger_Card)

With perfect forward secrecy

### It’s the internet.  
Verify your contacts.

![Mobile app screenshot showing the feature that allows users to mark a known contact as verified or send an identity verification request](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_1920,q_auto/Homepage/Screens/Messenger%20Feature%20Section/Messenger_02)

### Send crypto to your friends directly from chat.

![Mobile app screenshot showing a private chat between users: the messaging feature allows the user to send images, voice messages, crypto and much more](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_1920,q_auto/Homepage/Screens/Messenger%20Feature%20Section/Messenger_03)

Wallet

The future is multi-chain
-------------------------

L2s made simple - send and manage your crypto easily and safely across multiple networks.

[Learn more](https://status.im/features/wallet)

![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_256,q_auto/Homepage/Stickers/03)

![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_256,q_auto/Homepage/Stickers/04)

![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_256,q_auto/Homepage/Stickers/05)

### Send with automatic bridging. No more multi-chain hassle.

![Mobile app screenshot showing the wallet feature: the user is able to automatically bridge and manage tokens without any multi-chain hassle](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_1920,q_auto/Homepage/Screens/Wallet%20Feature%20Section/Wallet_01)

### Fully self-custodial. Nobody can stop you from using your tokens.

![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_3840,q_auto/Homepage/Illustrations/Wallet%20Feature%20Section/01_Wallet_Card)

### See how your total balances change over time, in fiat.

![Mobile app screenshot showing how the wallet feature shows your total balances changing overtime in fiat](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_1920,q_auto/Homepage/Screens/Wallet%20Feature%20Section/Wallet_02)

![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_3840,q_auto/Textures/screen-texture)

dApp Browser

Explore dApps
-------------

Interact trustlessly with web3 dApps, DAOs, NFTs, DeFi and much more.

![Mobile app screenshot showing how the Web3 browser interacts with NFT markets, allowing the user to buy a collectible directly from the app using the fully integrated wallet](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_1920,q_auto/Homepage/Screens/Browser%20Feature%20Section/Browser_01)

![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_3840,q_auto/Textures/screen-texture)

### Be free from tracking and data collection.

![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_1920,q_auto/Homepage/Illustrations/Browser%20Feature%20Section/01_Browser_Card)

### Omnichain dApp connections. So you don’t have to pick chains.

![Mobile app screenshot showing the Web3 browser that allows users to navigate and browse free without any tracking or data collection](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_1920,q_auto/Homepage/Screens/Browser%20Feature%20Section/Browser_03)

open source, decentralised crypto communication super app

open source, decentralised crypto communication super app

![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_96,q_auto/Homepage/Icons/Feature%20Section/01_Jump_To)

Jump between tasks

Like an operating system, whatever you’ve recently been doing is just a few taps away. Go from chatting with a friend to an account without having to navigate your way back.

Track your notifications

Make it your own

Use multiple profiles

![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_3840,q_auto/Textures/screen-texture)

Jump between tasks

Like an operating system, whatever you’ve recently been doing is just a few taps away. Go from chatting with a friend to an account without having to navigate your way back.

![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_3840,q_auto/Textures/screen-texture)

Track your notifications

With communities, 1on1 chats, wallet accounts and dApps, there is a lot going on! Stay in control with a global notification centre that lets you see at a glance everything that’s happening in your web3 world.

![Mobile app screenshot showing the global notification centre that lets the users see at a glance everything that's happening inside the app](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_1920,q_auto/Homepage/Screens/Features/02_Notifications)![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_3840,q_auto/Textures/screen-texture)

Make it your own

Customise your Status! Pick an avatar and colour to represent yourself, choose from light or dark mode, pick emojis to represent your accounts, and see the app will take on the character of the communities you join.

![Mobile app screenshot showing the customizing options that the app offers: users are able to pick an avatar and a colour to represent themselves, choose from light or dark mode, pick emoji to represent their accounts, and see the app take on the character of communities](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_1920,q_auto/Homepage/Screens/Features/03_Make_it_Your_Own)![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_3840,q_auto/Textures/screen-texture)

Use multiple profiles

Different online identities? Or sharing a device with family or friends? Not a problem, Status fully supports switching between multiple fully independent identities.

![Mobile app screenshot showing the multiple accounts feature that allows users to switch between multiple fully independent identities, a convenient feature if the user is sharing their devices between family and friends or has different online identities](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_1920,q_auto/Homepage/Screens/Features/04_Multiple_Profiles)![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_3840,q_auto/Textures/screen-texture)

Status is better with Keycard
-----------------------------

A secure contactless hardware wallet.

[Learn More](https://status.im/keycard)

### Decentralising the future

Building apps to uphold human rights, protect free speech & defend privacy.

[Our manifesto](https://status.im/manifesto)

### A token by and for Status

Participate in Status’ governance and help guide development with SNT.

[About SNT](https://status.im/token)

### Stay up to date

Follow development progress as we build a truly decentralised super app.

[Read the blog](https://status.im/blog)

![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_256,q_auto/Homepage/Stickers/06)

![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_256,q_auto/Homepage/Stickers/07)

![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_256,q_auto/Homepage/Stickers/08)

![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_1080,q_auto/Brand/Logo%20Section/Mark/Mark_01)

Be unstoppable
==============

Use the open source, decentralised crypto communication super app.

Mobile early accessBeta 2.0 for macOS[Beta 2.0 for Windows](https://status.im/api/download/windows)[Beta 2.0 for Linux](https://status.im/api/download/linux)

iOS & Android betas coming soon.  
Desktop beta 2.0 is live!

[![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_1080,q_auto/Brand/Logo%20Section/Mark/Mark_01)](https://status.im/)

Features

* [Communities](https://status.im/features/communities)
* [Create community](https://status.im/features/create-community)
* [Messenger](https://status.im/features/messenger)
* [Wallet](https://status.im/features/wallet)

Ecosystem

* [Apps](https://status.im/apps)
* [Keycard](https://status.im/keycard)

Organization

* [Manifesto](https://status.im/manifesto)
* [Team](https://status.im/team)
* [Brand](https://status.im/brand)
* [Jobs](https://status.im/jobs)

Help

* [Overview](https://status.im/help)
* [Getting started](https://status.im/help/getting-started)
* [Messaging](https://status.im/help/messaging)
* [Communities](https://status.im/help/communities)
* [Wallet](https://status.im/help/wallet)
* [Profile](https://status.im/help/profile)

Collaborate

* [Discuss](https://discuss.status.app/)
* [Feature upvote](https://status.im/feature-upvote)
* [Blog](https://status.im/blog)
* [Translations](https://status.im/translations)

Developers

* [Repos](https://github.com/status-im)
* [Insights](https://status.im/insights/epics)
* [Specs](https://status.im/specs)

SNT

* [Token](https://status.im/token)
* [Governance](https://governance.status.app/)
* [Curate](https://curate.status.app/)
* [Exchanges](https://status.im/exchanges)

[![](https://res.cloudinary.com/dhgck7ebz/image/upload/f_auto,c_limit,w_1080,q_auto/Brand/Logo%20Section/Mark/Mark_01)](https://status.im/)

Features

Ecosystem

Organization

Help

Collaborate

Developers

SNT

Legacy mobile apps

While you wait for Status Mobile 2.0 you can use our previous mobile apps.

Install Status Mobile 1.20

© Status Research & Development GmbH

[Terms of Use](https://status.im/legal/terms-of-use)[Privacy Policy](https://status.im/legal/privacy-policy)[Security](https://status.im/security)

[Messari Transparency Verified](https://messari.io/asset/status)

[![Twitter](/_next/static/media/twitter.c69fbd1a.svg)](https://twitter.com/ethstatus)[![YouTube](/_next/static/media/youtube.69d4dc12.svg)](https://www.youtube.com/@Statusim)[![GitHub](/_next/static/media/github.34ba96c8.svg)](https://github.com/status-im)
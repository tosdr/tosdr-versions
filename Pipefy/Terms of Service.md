...
|     |     |     |     |     |     |
| --- | --- | --- | --- | --- | --- |
| **Plans** | **Level of Criticality** | **First Response Time** | **Resolution Time³**<br><br>**Low Complexity** | **Resolution Time³**<br><br>**High Complexity** | **Availability** |
| **Business** | Service Outage | 1 hour | 6 hours | 6 hours (24/7) | 12 hours/5 days<br><br>8 AM to 8 PM  <br>UTC -3 |
...
| **Definitions:**<br><br>**i) Level of Criticality:**<br><br>* **Service Outage:** Total or partial outage of the Pipefy Solution. Important services/components are non-functional and affect the Pipefy production environment of multiple Customers.<br>* **High**: Severe impact on performance. Important services/components are non-functional, a single connection is down, or a subset of users cannot sign in to Pipefy's production environment.<br>* **Medium**: Low impact on a small number of users in a production environment.<br>* **Low**: User questions, enhancement feedback, and specific requests (Reports, Restoring Deleted Fields, Cards or Pipes, Resending invites, and Password Recovery).<br><br>**ii) First Response Time:** The time elapsed between the initiation of a chat by a user and the first response from an agent. We also provide support via email at [\[email protected\]](https://www.pipefy.com/cdn-cgi/l/email-protection) during business hours; however, this channel does not guarantee a first response time.<br><br>**iii) Resolution Time:**<br><br>* **Low Complexity:** The time it takes to resolve low-complexity issues requiring knowledge of the Pipefy tool, such as—but not limited to—questions, misuse, identifying and reporting bugs, and troubleshooting usage problems, is typically handled by our Level 1 Customer Support team within a single chat.<br>* **High Complexity:** The time it takes to resolve high-complexity issues requiring technical and systems knowledge, such as—but not limited to—API assistance, identifying and troubleshooting technical problems in the system, monitoring system health, and developing and testing solutions for prioritized bugs.<br><br>_iv) This table does not cover the handling of bugs, as these events undergo a separate analysis and prioritization process. Deadlines and justifications for resolving bugs are determined individually based on the impact and number of users affected._ |     |     |     |     |     |
...
| --- | --- |
| **Plan** | **Plan Coverage** |
| **Starter** | (a) Up to 320,000 requests per day of a set of routines and programming standards for accessing a web-based software application or platform (“API”) received in Pipefy’s central infrastructure, limited to a maximum transaction per second (“TPS”) of 160 requests. This includes requests from external .NET solutions calling the Pipefy API or requests to the Automation Subprocessor calling the Pipefy API, both of which count towards the daily quota;<br><br>(b) Up to 4 million (4,000,000) automation jobs, defined as automation configurations executed in Pipefy’s infrastructure within a month;<br><br>(c) Up to 500 GB of storage for saving attachments from electronic forms or cards. |
| **Corporate** | (a) Up to 500,000 requests per day of a set of routines and programming standards for accessing a web-based software application or platform (“API”) received in Pipefy’s central infrastructure, limited to a maximum transaction per second (“TPS”) of 250 requests. This includes requests from external .NET solutions calling the Pipefy API or Automation Subprocessor requests calling the Pipefy API, both counted towards the daily quota;<br><br>(d) Up to 6.5 million (6,500,000) automation jobs, defined as automation configurations executed in Pipefy’s infrastructure within a month;<br><br>(c) Up to 750 GB of storage for saving attachments from electronic forms or cards. |
| **Enterprise** | (a) Up to 750,000 API requests received per day in Pipefy’s central infrastructure, limited to a maximum transaction per second (TPS) of 375 requests. This includes requests from external .NET solutions calling the Pipefy API or Automation Subprocessor requests calling the Pipefy API, both counted towards the daily quota;<br><br>(b) Up to 9 million (9,000,000) automation jobs, defined as automation configurations executed in Pipefy’s infrastructure within a month;<br><br>(c) Up to 1,024 GB of storage for saving attachments from electronic forms or cards. |
| **No plan contemplates quotas for inbound or outbound traffic based on Automation Subprocessor Integrations.** |     |
...
(b) Automation Subprocessor does not include a _staging_ environment and usage for testing cannot be excluded from billing calculations as there is no efficient way to separate it.

...
| Name | Purpose | Expiration |
| --- | --- | --- |
| hjIncludedInSessionSample | Hotjar cookie set to determine if a user is included in the data sampling defined by your site's daily session limit. | When user closes browser |
| \_hjIncludedInPageviewSample | Hotjar cookie set to determine if a user is included in the data sampling defined by your site's pageview limit. | When user closes browser |
| \_hjAbsoluteSessionInProgress | Hotjar cookie used to detect the first pageview session of a user. | When user closes browser |
| \_hjFirstSeen | Hotjar cookie used by Recording filters to identify new user sessions. | When user closes browser |
| \_dc\_gtm\_# | Google Tag Manager cookie used to analyse how users interact with the website. | 1 day |
...
| \_hjSession\_# | Hotjar cookie to ensure subsequent requests in the session window are attributed to the same session. | 1 year |
...
| ai\_user | Microsoft cookie to collect statistical usage of Power BI data reports. | 1 year |
| \_ga | Google Analytics cookie to collect data on website traffic. Read a summary of Google Analytics’ data practices. | 2 years (Chrome's maximum of 400 days and Safari's maximum of 7 days.) |
| SAPISID, APISID, HSID | Google Analytics cookie used for customisation. | 2 years |

Preference/Functional cookies
-----------------------------

These cookies provide you with a more tailored experience based on your visit.

| Name | Purpose | Expiration |
| --- | --- | --- |
| EPiStateMarker | This cookie is functionality-related. Indicates how session-based information on the visit should be stored, using sessions or cookies. | Session |
| is  | This cookie identifies a visitor's session. This cookie is used in conjunction with the iv cookie, and is erased when the visitor closes the browser. | Session |
...
| iv  | This cookie tracks the anonymous visitor's actions across the the site  for Content recommendation keywords for specific user, that helps Content recommondation engine to track user journey. This helps us provide content recommendations based on the visit | Persistent |
...
| visitor\_info1\_live | Youtube cookie used to to provide bandwidth estimations. | 1 month |
| device\_info | Youtube cookie used to track user interaction with embeded content. | 5 months |
| NID | The ‘NID’ cookie is used to show Google ads in Google services for signed-out users, while the ‘ANID’, ‘IDE’ and ‘id’ cookies are used to show Google ads on non-Google sites. | 6 months |
| ajs\_anonymous\_id | JIRA cookie used to store last visits to a website | 1 year |
| \_fw\_crm\_v | This cookie holds the tracker id Freshworks use to identify the user who is coming back on to the chat widget for the embedded Freshworks form on the contact page​. It is required to enable the site to function. This cookie is provided by Freshworks. Please view their privacy statement here - https://www.freshworks.com/privacy/ | 1 year |
